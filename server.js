...
// Serve admin static files first, then main static
...
function search(query, opts={}){
  ...
  let pool = db.all;
  if(category==='library' && subcat) pool = db.library.filter(d=>d.subcat===subcat);
  else if(category) pool = db.all.filter(d=>d.category===category);
...
function findSections(query, limit=3){
  ...
  return results.map(r=>{
    ...
    return { id:r.id, title:r.title, category:r.category, score:r.score, excerpt:bestPara.substring(0,600), totalLength:doc.content.length };
  }).filter(Boolean);
}

function generateResponse(query, opts={}){
  const sections = findSections(query, opts.limit||4);
  if(!sections.length) return { answer:'عذراً، لم أجد معلومات في قاعدة البيانات حول هذا السؤال.', sources:[], steps:['بحث في قاعدة البيانات'] };

  const catNames = {laws:'القوانين',library:'الدعاوى والإجراءات',contracts:'العقود',articles:'المقالات'};
  const steps = ['تحليل الاستشارة القانونية', `البحث في ${db.all.length} وثيقة`, `وجدت ${sections.length} مراجع`, 'إعداد الرد'];

  let answer = 'بناءً على مراجعة القاعدة القانونية اليمنية:\n\n';
  sections.forEach((s,i)=>{
    if(i>0) answer += '─────────────────\n';
    answer += `${s.title}\n${s.excerpt.substring(0,400)}${s.excerpt.length>400?'...':''}\n\n`;
  });
  answer += '─────────────────\n⚠️ تنبيه: معلومات عامة من القوانين اليمنية. راجع محامٍ متخصص.';

  return { answer, sources:sections.map(s=>({title:s.title,category:catNames[s.category]||s.category,score:s.score,id:s.id})), steps };
}
...
app.post('/api/chat', (req, res) => {
  ...
  setTimeout(()=>res.json(generateResponse(message, {category,subcat})), 600);
});
...
app.get('/api/stats', (req, res) => res.json({ laws:db.laws.length, library:db.library.length, contracts:db.contracts.length, articles:db.articles.length, total:db.all.length, totalChars:db.all.reduce((s,d)=>s+d.content.length,0) }));
app.get('/api/laws', (req, res) => res.json(db.laws.map(d=>({id:d.id,title:d.title,length:d.content.length}))));
app.get('/api/library/subcategories', (req, res) => { const o={}; for(const[k,v] of Object.entries(libSubcats)) o[k]={count:v.length,items:v.map(i=>({id:i.id,title:i.title}))}; res.json(o); });
app.get('/api/library/:subcat', (req, res) => { const s=decodeURIComponent(req.params.subcat); res.json({subcat:s,count:db.library.filter(d=>d.subcat===s).length,items:db.library.filter(d=>d.subcat===s).map(d=>({id:d.id,title:d.title,length:d.content.length}))}); });
app.get('/api/contracts/types', (req, res) => { const o={}; for(const[k,v] of Object.entries(contractTypes)) o[k]={count:v.length,items:v.map(i=>({id:i.id,title:i.title}))}; res.json(o); });
app.get('/api/contracts/:type', (req, res) => { const t=decodeURIComponent(req.params.type); res.json({type:t,count:db.contracts.filter(d=>d.type===t).length,items:db.contracts.filter(d=>d.type===t).map(d=>({id:d.id,title:d.title,length:d.content.length}))}); });
app.get('/api/articles', (req, res) => res.json(db.articles.map(d=>({id:d.id,title:d.title,length:d.content.length}))));
app.post('/api/contracts/generate', (req, res) => {
  ...
  if(fields) for(const[k,v] of Object.entries(fields)) content = content.replace(new RegExp(`\\{\\{${k}\\}\\}`,'g'), v);
